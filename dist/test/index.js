...
import { deepCopy, deepMerge, deepMergeArrays, wait, deepEqual, toEnvVar, readStream, queued, retry, randomString, getType, padLeft, padRight, setByPath, getByPath, } from '../src/index.js';
...
test('readStream', async (t) => {
    ...
    const v = await readStream(createReadStream(join(__dirname, '../../package.json')));
...
test('retry', async (t) => {
    ...
    i = 0;
    async function failsTwiceWithArgs(v) {
        if (i < 2) {
            i++;
            throw new Error('no');
        }
    ...
    await t.throwsAsync(retry(failsTwiceWithArgs, { timeout: 100, maxRetries: 1 }, 'hello'));
    i = 0;
    const res3 = await retry(failsTwiceWithArgs, { timeout: 100, maxRetries: -1 }, 'hello');
    t.assert(res3 === 'hello');
    i = 0;
    d = Date.now();
...
test('randomString', (t) => {
    ...
    t.assert(upperCaseChars.split('').some((v) => {
        return (one.indexOf(v) >= 0 &&
            two.indexOf(v) >= 0 &&
            three.indexOf(v) >= 0 &&
            four.indexOf(v) < 0 &&
            five.indexOf(v) >= 0);
    }));
    t.assert(lowerCaseChars.split('').some((v) => {
        return (one.indexOf(v) >= 0 &&
            two.indexOf(v) >= 0 &&
            three.indexOf(v) < 0 &&
            four.indexOf(v) >= 0 &&
    ...
    t.assert(numberChars.split('').some((v) => {
        return (one.indexOf(v) >= 0 &&
            two.indexOf(v) >= 0 &&
            three.indexOf(v) >= 0 &&
            four.indexOf(v) >= 0 &&
    ...
    t.assert(specialsChars.split('').some((v) => {
        return (one.indexOf(v) >= 0 &&
            two.indexOf(v) < 0 &&
            three.indexOf(v) >= 0 &&
            four.indexOf(v) >= 0 &&
